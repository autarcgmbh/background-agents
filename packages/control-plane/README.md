...
- `integration_environment_settings`: environment-level integration-setting overrides (sandbox,
  ...
  (per-repo).
- `session_model_usage` and `sessions.total_tokens`: legacy usage analytics storage, retained for
  migration compatibility and historical data. New agent usage and conversations are exported to
  [Grafana Agent Observability](../../docs/AGENT_OBSERVABILITY.md).
...
Deploy D1 migration `0079_session_status_revision.sql` before the worker update. Runtime schema
