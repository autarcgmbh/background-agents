...
describe("AnthropicModelProviderAccountAdapter", () => {
  ...
  it("exposes no runtime metadata and only rejects contradicting identities", () => {
    ...
    const observed = (externalAccountId?: string) => ({ externalAccountId });
    const stored = (externalAccountId: string | null) => ({
      externalAccountId,
      externalPrincipalId: null,
    });
    expect(() => adapter.validateExternalIdentity(observed(), stored(null))).not.toThrow();
    expect(() => adapter.validateExternalIdentity(observed(), stored("acct"))).not.toThrow();
    expect(() => adapter.validateExternalIdentity(observed("acct"), stored(null))).not.toThrow();
    expect(() => adapter.validateExternalIdentity(observed("a"), stored("b"))).toThrow(
      /did not match/
    );
  });
