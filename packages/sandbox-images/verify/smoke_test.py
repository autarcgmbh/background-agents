...
def inspect_image(plan: dict[str, Any], tools: dict[str, Any], *, services: bool) -> None:
    ...
    if plan["target"].get("workload") == "autarc":
        for command in (
            "go",
            "gofmt",
            "golangci-lint",
            ...
            probe.run(
                ["python3", "-c", "import shutil,sys; assert shutil.which(sys.argv[1])", command]
            )
        ...
        probe.run(
            [
                "python3",
                "-c",
                "import os,pathlib; home=pathlib.Path.home(); assert (home / '.agents/skills/debug-with-grafana').is_dir(); assert (home / '.agents/skills/stripe-best-practices/SKILL.md').is_file(); assert 'posthog-cli' in (home / '.config/opencode/AGENTS.md').read_text(); assert all(os.access(p, os.W_OK) for p in ('/opt/pnpm-home','/opt/pnpm-store','/opt/turbo-cache','/opt/go'))",
            ]
        )
