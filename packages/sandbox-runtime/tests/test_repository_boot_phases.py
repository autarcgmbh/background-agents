...
class TestHookFailureTail:
    ...
    async def test_a_build_mode_failure_does_not_report_an_earlier_tail(self, tmp_path):
        ...
        # A build keeps its own tail out of the provider's log, but still
        # records it: the build-failed callback is the only way it reaches the
        # control plane, since boot phases are suppressed in build mode.
        script.write_text("#!/bin/bash\necho later\nexit 1\n")
        assert await sup.hooks.run_setup(repo, BootMode.BUILD) is False

        assert sup.hooks.failure_tail(repo, "setup") == ("later",)
